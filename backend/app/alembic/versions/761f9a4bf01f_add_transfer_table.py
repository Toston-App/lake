...
def upgrade():
    ...
    op.add_column('account', sa.Column('total_transfers_out', sa.Float(), nullable=True))
    op.drop_index('ix_account_total_transfers', table_name='account')
    op.create_index(op.f('ix_account_total_transfers_in'), 'account', ['total_transfers_in'], unique=False)
    op.create_index(op.f('ix_account_total_transfers_out'), 'account', ['total_transfers_out'], unique=False)
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('account', sa.Column('total_transfers', postgresql.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True))
    ...
    op.create_index('ix_account_total_transfers', 'account', ['total_transfers'], unique=False)
