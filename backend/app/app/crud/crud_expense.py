...
class CRUDExpense(CRUDBase[Expense, ExpenseCreate, ExpenseUpdate]):
    async def create_with_owner(
        ...
        if obj_in_data['account_id']:
            update = await crud.account.update_by_id_and_field(db=db, id=obj_in_data['account_id'], column='total_expenses', amount=obj_in_data['amount'])
        ...
        await crud.user.update_balance(db=db, user_id=owner_id, is_Expense=True, amount=obj_in_data['amount'])
