...
def gen_accounts(n):
    ...
    for _ in range(n):
        ...
        account["initial_balance"] = fake.pyfloat(
            left_digits=5, right_digits=2, min_value=0
        )
...
def gen_expenses(n):
    ...
    for _ in range(n):
        expenses = {}
        expenses["description"] = fake.paragraph(
        ...
        expenses["date"] = fake.date_between(
...
def gen_transfers(n):
    ...
    for _ in range(n):
        transfers = {}
        transfers["description"] = fake.paragraph(
            nb_sentences=1, variable_nb_sentences=True, ext_word_list=None
        )
...
gen_accounts(
    10
)  # If this value is changed, change the max value in account in gen_expenses, incomes and transfers
gen_places(10)
gen_expenses(10)
gen_incomes(10)
gen_transfers(30)
