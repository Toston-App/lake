...
async def read_transactions(
    ...
    """
    Retrieve paginated transactions (expenses, incomes, transfers) with filtering.
    Supports search, amount comparison, date range, account/category/place filters,
    ...
    active_filters = {
        ...
        for k, v in {
            ...
            "amount": amount,
            "amount_operator": (
                amount_operator.value if amount_operator else None
            ),
            ...
            "places": places,
            "transaction_type": (
                [t.value for t in transaction_type] if transaction_type else None
            ),
        }.items()
        if v is not None
    }
    ...
    enrich_event(
        request,
        database={
            "operation": "v3_transactions",
            "duration_ms": t.ms,
            "results_count": (
                len(transactions.items) if hasattr(transactions, "items") else 0
            ),
        },
    )
