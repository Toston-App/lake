...
class CurrencyRateUnavailable(RuntimeError):
    """Raised when no fresh, trustworthy FX rate is available."""
...
class CurrencyConverter:
    ...
    @classmethod
    async def get_usd_to_mxn_rate(cls) -> Decimal:
        ...
        try:
            ...
            if not hist.empty:
                ...
                if not rate.is_finite() or rate <= 0 or rate > Decimal("1e6"):
                ...
                return rate
            raise CurrencyRateUnavailable("No USD/MXN market data returned")
        except Exception as e:
            logger.error(f"Error fetching USD/MXN rate: {e}")
            raise CurrencyRateUnavailable("USD/MXN rate is unavailable") from e

    @classmethod
    async def get_mxn_to_usd_rate(cls) -> Decimal:
        """Get current MXN to USD exchange rate."""
        usd_mxn = await cls.get_usd_to_mxn_rate()
        return Decimal("1") / usd_mxn

    @classmethod
    async def convert_usd_to_mxn(cls, amount: Decimal) -> Decimal:
        """Convert USD amount to MXN."""
        rate = await cls.get_usd_to_mxn_rate()
        return amount * rate

    @classmethod
    async def convert_mxn_to_usd(cls, amount: Decimal) -> Decimal:
    ...
    @classmethod
    async def convert(
        cls, amount: Decimal, from_currency: str, to_currency: str
    ) -> Decimal:
        """
