# Wallet - Financial Tracking Application

Wallet is an efficient financial tracking application designed to help you effectively manage your expenses ensuring the privacy and security of your financial data. It offers various features to record, categorize, and generate reports on your expenses and incomes.

## Features

- Expense Logging: Quickly and effortlessly record your expenses.
...
JSON based web API based on OpenAPI: http://localhost/api/
...
If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.wallet.com`).
...
```bash
STACK_NAME=stag-wallet-com sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.wallet-com.app-db-data == true
```
...
```bash
docker node update --label-add wallet-com.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-wallet-com.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=wallet.com`
  * `TRAEFIK_TAG=wallet.com`
  * `STACK_NAME=wallet-com`
  * `TAG=prod`
...
Frontend: https://wallet.com

Backend: https://wallet.com/api/

Automatic Interactive Docs (Swagger UI): https://wallet.com/docs

Automatic Alternative Docs (ReDoc): https://wallet.com/redoc
...
Frontend: https://stag.wallet.com

Backend: https://stag.wallet.com/api/

Automatic Interactive Docs (Swagger UI): https://stag.wallet.com/docs

Automatic Alternative Docs (ReDoc): https://stag.wallet.com/redoc
...
Frontend: http://dev.wallet.com

Backend: http://dev.wallet.com/api/

Automatic Interactive Docs (Swagger UI): https://dev.wallet.com/docs

Automatic Alternative Docs (ReDoc): https://dev.wallet.com/redoc
